...
"A05": [
 ...
 ("Culture: CAMS (Culture, Automation, Measurement, Sharing), Agile vs DevOps, 6 phases of DevOps lifecycle",
  "CAMS: Culture (collab over blame), Automation (repeatable), Measurement (data-driven), Sharing (knowledge/ownership). Agile vs DevOps: Agile is about how teams build (iterations, feedback); DevOps extends to operations — how software is delivered and run (culture + tooling across dev and ops). 6-phase lifecycle: Plan → Code → Build → Test → Release → Deploy → Operate → Monitor (loop).",
